...
COMPARE_QUESTION_BUILDER_PROMPT_3WAY = """
You are generating a comparison question involving THREE entities.

Entity A:
{subject_entity_name} ({subject_entity_type})
...
Attributes of A:
{attributes_list_str_a}

{target_info}

### Task
Using the above documents, write a comparison question that asks:

**"Among A, B, and C, which one has the larger/better value of the target attribute?"**
...
---Instructions---
You are a Polisher module responsible for optimizing comparison questions. Based on the input of three entities (A, B, and C), the attribute being compared, supporting facts, the original question-answer pair, and relevant document contexts, you need to evaluate the quality of the question and make one of the following four decisions:
...
3. If the question needs substantial rewriting:
   ...
   REFINED_FACT_C: [If needed, corrected fact for entity C]
   ```
...
Fact for Entity C: {fact_entity_c}
...
**Imagine you are comparing documents, Document A (about Entity A), Document B (a candidate potentially containing a related Entity B), and Document C (another candidate potentially containing a related Entity C).** Your task is to:
1.  Identify the main subject entity within Document B (potential Entity B) and Document C (potential Entity C) and see if they are relevant to Entity A.
2.  Find if there is **at least one specific, comparable attribute pair** between Entity A (using its provided attribute list) and the potential Entity B (using Document B), and the **same attribute** between Entity A and the potential Entity C (using Document C).
3.  If a suitable comparison pair is found for **both** Entity B and Entity C, **directly generate** a natural language **direct comparison question**, its concise **comparative answer**, and the specific **full sentence(s)** from each document (A, B, and C) supporting the values being compared.
4.  If no suitable entity or comparable attribute pair is found in Document B or Document C, indicate failure.
...
*   **Candidate Document C Text:** **{document_c_text}** (Another document to analyze)
{target_info}

2.  **Identify Entity B, Entity C, and Find ONE Comparable Attribute Pair:**
...
*   b. Identify the primary subject entity within `document_c_text`. Let's call it **Entity C**. If no clear Entity C is found, or if it seems entirely irrelevant to Entity A, STOP and proceed to step 4 (Failure).
*   c. Check if Entity B's type and Entity C's type are generally compatible/relevant for comparison with Entity A's type (`{subject_entity_type}`). If not, STOP and proceed to step 4 (Failure).
*   d. Iterate through Entity A's attributes in `attributes_list_str_a`. For each attribute `A_X` (Name: `A_Name`, Value: `A_Value`):
    ...
    *   Search `document_c_text` to see if Entity C has a corresponding attribute `C_X` (same or semantically equivalent name) with a specific value `C_Value`.
    *   Only consider attributes where **all three values** (`A_Value`, `B_Value`, `C_Value`) are present and **clearly comparable** (e.g., all dates, all numbers, all specific locations, all clear categories; values should be concise and factual, not vague or overly long descriptions).
*   e. If you find **the first such comparable triple** (`A_Name`, `A_Value`, `B_Value`, `C_Value`), **STOP iterating**. Record:
    *   `A_Name` as `matched_attribute_name`
    *   `A_Value` as `value_a`
    *   the identified Entity B name as `entity_b_name`
    *   `B_Value` as `value_b`
    *   the identified Entity C name as `entity_c_name`
    *   `C_Value` as `value_c`
    Proceed to step 3 (Success).
*   f. If no such attribute with comparable values for A, B, and C is found, proceed to step 4 (Failure).

3.  **Generate DIRECT Comparison Question, COMPARATIVE Answer, and Facts (If a Triple Was Found in Step 2e):**
*   a. Compare `value_a`, `value_b`, and `value_c` to determine their precise relationship (e.g., which is greater/earlier/later, or whether some/all are equal).
*   b. **Generate DIRECT Comparison Question:** Create a clear, natural question that **explicitly asks for the result of the comparison** about the `matched_attribute_name` among the three entities: the subject entity (Entity A), Entity B, and Entity C.  
    The question MUST elicit a comparative answer (like which entity is greater/earlier/more, or whether they share the same value), NOT simply ask for all three values.
    *   For example: "Which of [Entity A], [Entity B], and [Entity C] has the earlier [attribute name]?"  
    or: "Among [Entity A], [Entity B], and [Entity C], who has the highest [attribute name]?"
*   c. **Generate COMPARATIVE Answer:** Provide the **direct, concise answer** that directly answers the comparative question generated in step 3b.  
    The answer MUST be the *result* of the comparison, not a restatement of all input values.  
    Typical answers: a single entity name (e.g., "[entity_a_name]" / "[entity_b_name]" / "[entity_c_name]") or a short comparative phrase if needed (e.g., "They share the same value" if all three are equal).
*   d. **Extract Supporting Facts:** Extract the full sentence(s) from `document_a_text` supporting `value_a`, the full sentence(s) from `document_b_text` supporting `value_b`, and the full sentence(s) from `document_c_text` supporting `value_c` for the `matched_attribute_name`.
*   e. **Extract Relevant Paragraphs:** Extract relevant paragraphs (50-150 words) directly from each of the three documents that provide context for the comparison question. These paragraphs MUST contain the most pertinent information related to the compared attribute.  
    DO NOT just return the title or a single line - you MUST return a complete paragraph with substantive text from each document, even if part of the document seems unrelated to the entity.

4.  **Indicate Failure (If No Triple Was Found in Step 2 or Initial Checks Failed):**
*   If you stopped in step 2a, 2b, 2c, or completed 2d without finding any comparable attribute values for A, B, and C, the output should be `FAIL`.

### ---Output Format Specification---

**Strictly adhere to the following output format:**

1.  **Success Output (If a comparable attribute triple was found):**
...
entity_c: Identified Entity C Name (from step 2)
...
fact_entity_c: Extracted Full Sentence(s) for Fact C (from step 3)
...
relevant_paragraph_c: Complete substantive paragraph (50-150 words) from Document C (not just the title)
```
2.  **Failure Output (If no comparable attribute triple or valid Entity B/C found):**
