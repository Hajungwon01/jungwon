        ...
        try:
            ...
            for result_index, result_data in enumerate(retrieval_list):
                ...
                if len(retrieved_docs) < 2:
                    self.logger.warning(
                        "Not enough retrieved documents for B and C (got {}), skipping this retrieval result".format(
                            len(retrieved_docs)
                        )
                    )
                    continue
                ...
                # TOP2 → B, C로 사용
                doc_b_data = retrieved_docs[0]
                doc_c_data = retrieved_docs[1]

                self.logger.info(
                    "Processing retrieval result {}/{} using top-2 docs as B and C (total retrieved: {})".format(
                        result_index + 1, len(retrieval_list), len(retrieved_docs)
                    )
                )

                # ----- B 문서 파싱 -----
                id_field_b = "doc_id" if "doc_id" in doc_b_data else "id"
                content_field_b = None
                for field in ["contents", "content", "text"]:
                    if field in doc_b_data and doc_b_data.get(field):
                        content_field_b = field
                        break

                doc_b = {
                    "id": doc_b_data.get(id_field_b, ""),
                    "title": doc_b_data.get("title", ""),
                    "contents": doc_b_data.get(content_field_b, "") if content_field_b else ""
                }

                # ----- C 문서 파싱 -----
                id_field_c = "doc_id" if "doc_id" in doc_c_data else "id"
                content_field_c = None
                for field in ["contents", "content", "text"]:
                    if field in doc_c_data and doc_c_data.get(field):
                        content_field_c = field
                        break

                doc_c = {
                    "id": doc_c_data.get(id_field_c, ""),
                    "title": doc_c_data.get("title", ""),
                    "contents": doc_c_data.get(content_field_c, "") if content_field_c else ""
                }

                if not doc_b["contents"] or not doc_c["contents"]:
                    self.logger.warning(
                        "Document B ({}) or C ({}) has empty contents, skipping".format(
                            doc_b["id"], doc_c["id"]
                        )
                    )
                    continue

                # ✅ 이제 builder를 B, C 둘 다 넘기는 버전으로 호출
                question_data = self.question_builder.build_question(
                    entity_data=filtered_entity_data,
                    candidate_doc_b=doc_b,
                    candidate_doc_c=doc_c,
                )

                if question_data and question_data.get("success", False):
                    comparison_questions.append(question_data)
                    self.logger.info(
                        "Successfully built comparison question using documents B={} and C={}".format(
                            doc_b["id"], doc_c["id"]
                        )
                    )
                    # 한 번 성공하면 이 retrieval_result는 더 안 봄
                    break
